...
- [Методы генерации кода](#code-generate)
    ...
    - [Создание из Eloquent модели](#model)
...
Команда имеет следующую сигнатуру `moonshine:build {target?} {--type=}`, где:
 ...
 - `type` - тип или метод генерации, доступно `table`, `json`, `model`, `console`.
...
<a name="model"></a>
### Создание из Eloquent модели

Вы можете создать ресурс на основе существующей Eloquent модели. Для этого выполните команду `php artisan moonshine:build` и выберите вариант `model`:
```shell
 ┌ Type ────────────────────────────────────────────────────────┐
 │   ○ table                                                    │
 │   ○ json                                                     │
...
Выберите модель из списка. Первым пунктом доступен вариант `All` для генерации ресурсов сразу для всех моделей:
```shell
 ┌ Model ───────────────────────────────────────────────────────┐
 │ › ● All                                                      │
 │   ○ Category                                                 │
...
При генерации из модели автоматически определяются:
- Поля таблицы и их типы
- Связи (BelongsTo, HasMany, HasOne, BelongsToMany)
- Касты полей

При этом файлы Model и Migration не создаются, так как модель уже существует.

Вы можете сразу указать тип генерации:
```shell
php artisan moonshine:build --type=model
```
